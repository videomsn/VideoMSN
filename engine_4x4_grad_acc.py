...
def train_one_epoch(model: torch.nn.Module, target_model: torch.nn.Module, run, wandb, criterion: torch.nn.Module,
                    ...
                    memax_weight=1.0, ent_weight=0.0, temporal_aug=False, grad_accum_steps=2
                    ):
    ...
    for step, (samples, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=amp):
            ...
            outputs_anchor = model.forward(samples_list[1:], msn_pretraining, temporal_aug)
            with torch.no_grad():
        ...
        if amp:
            ...
            loss_scaler(
                ...
                parameters=model.parameters(), create_graph=is_second_order,
                update_grad=(step + 1) % grad_accum_steps == 0  # <<< accumulate
            )
        else:
